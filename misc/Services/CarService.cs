...
namespace BlazorApp1.Services
{
    public class CarService : ICarService
    {
        ...
        public CarService(ICarRepository carRepository, IEventRepository eventRepository, ICarStaticDetailRepository carStaticDetailRepository)
        {
        ...
        {
            var cars = await _carRepository.GetAll_();

            foreach (var car in cars)
            {
                if (car != null)
                {
                    var carStaticDetail = await _carStaticDetailRepository.GetCarStaticDetailByIdAsync(car.CarId);
                    if (carStaticDetail != null)
                    {
                        car.CarStaticDetail = carStaticDetail;
                    }
                    else
                    {
                        // Handle case where no CarStaticDetail was found for this CarId.
                        Console.WriteLine($"No CarStaticDetail found for CarId: {car.CarId}");
                        var otherCarStaticDetails = await _carStaticDetailRepository.GetAllCarStaticDetailsAsync();
                        foreach (var detail in otherCarStaticDetails)
                        {
                            Console.WriteLine($"CarId: {detail.CarId}, Tag: {detail.Tag}, Finas: {detail.Finas}, Vin: {detail.Vin}, Adas: {detail.Adas}, Id: {detail.Id}");
                        }
                        // Handle case where no CarStaticDetail was found for this CarId.
                    }
                }
